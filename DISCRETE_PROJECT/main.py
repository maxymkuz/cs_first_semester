        ...
        if check_if_ok(board, row, column):
            ...
            if column == n-1:
                if board not in lst_of_results:
                    lst_of_results.append(copy.deepcopy(board))
                board[row][column] = 0
                continue
...
def duplicate_filter(n, solutions):
    ...
    for solution in solutions[1:]:
        ...
        # If solution is not dublicate, we save it to results list
...
def main():
    ...
    """
    # Inputing an n and checking if it's an integer > 0
    while True:
    ...
    if n < 4:
        if n == 0:
            print(f'Technically, there are infinite solutions for n = {n}')
        else:
            print(f'There are no solutions for n = {n}.')
        return
